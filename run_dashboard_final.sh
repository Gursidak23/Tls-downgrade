#!/usr/bin/env bash
# Launch the dashboard only (without re-running scans), using the project virtual environment.
...
exec "$PYTHON" "$SCRIPT_DIR/dashboard.py" --host "$HOST" --port "$PORT" --no-debug
